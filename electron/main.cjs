...
function registerIpc(mainWindow) {
  ...
  ipcMain.handle("erp:runHealthCheck", async (event, { schema, table, maxPerColumn = 500 } = {}) => {
    ...
    for (let i = 0; i < tables.length; i++) {
      ...
      // Get text columns with finite max length.
      ...
      const pkSelect = pk.length
        ? pk.map(quoteIdent).join(" + '|' + ")
        : null;

      for (const c of cols) {
        if (signal.aborted) { aborted = true; break; }
        const colIdent = quoteIdent(c.columnName);
        const idExpr = pkSelect
          ? `CAST(${pk.map((p) => `ISNULL(CAST(${quoteIdent(p)} AS NVARCHAR(MAX)),'')`).join(" + '|' + ")} AS NVARCHAR(MAX))`
          : `NULL`;
        try {
          const r = await pool.request()
